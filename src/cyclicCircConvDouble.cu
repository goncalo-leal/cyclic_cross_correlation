...
static void cyclicCircConv_cpu_kernel (double *x, double *y, double *xy, unsigned int nSamp);
__global__ static void cyclicCircConv_cuda_kernel (double *x, double *y, double *xy, unsigned int nSamp);
...
int main (int argc, char **argv)
{
  ...
  double *host_x, *host_y, *host_xy;

  host_x = (double *) malloc (N * sizeof (double));
  host_y = (double *) malloc (N * sizeof (double));
  host_xy = (double *) malloc (N * sizeof (double));

  // initialize signal datacyclicCircConv_cuda_kernel
  (void) get_delta_time ();
  srand(0xACA2020);
  for (i = 0; i < N; i++)
  { host_x[i] = (double) ((double) rand () / RAND_MAX - 0.5);
    host_y[i] = (double) ((double) rand () / RAND_MAX - 0.5);
  }
  ...
  double *device_x, *device_y, *device_xy;;

  (void) get_delta_time ();
  CHECK (cudaMalloc ((void **) &device_x, N * sizeof (double)));
  CHECK (cudaMalloc ((void **) &device_y, N * sizeof (double)));
  CHECK (cudaMalloc ((void **) &device_xy, N * sizeof (double)));
  CHECK (cudaMemcpy (device_x, host_x, N * sizeof (double), cudaMemcpyHostToDevice));
  CHECK (cudaMemcpy (device_y, host_y, N * sizeof (double), cudaMemcpyHostToDevice));
  printf ("The creation and transfer of %ld bytes from the host to the device took %.3e seconds\n",
          2 * (long) N * sizeof (double), get_delta_time ());
  ...
  double *modified_device_xy;

  (void) get_delta_time ();
  modified_device_xy = (double *) malloc (N * sizeof (double));
  CHECK (cudaMemcpy (modified_device_xy, device_xy, N * sizeof (double), cudaMemcpyDeviceToHost));
  printf ("The transfer of %ld bytes from the device to the host took %.3e seconds\n",
          (long) N * sizeof (double), get_delta_time ());
...
__global__ static void cyclicCircConv_cuda_kernel (double *xx, double *yy, double *xxyy, unsigned int nSamp)
{
